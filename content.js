(() => {
   ...
   * @param {string} align -> text alignment ('left', 'center', etc.)
   * @returns {HTMLElement}
   */
  ...
  function createPlaylistsBox(playlists) {
    if (!playlists || playlists.length === 0) return null;

    const box = document.createElement('div');
    box.className = 'yt-enhanced-info-item';
    box.textContent = '☰♪ Playlists';
    styleInfoBox(box);
  ...
  async function updateInfoColumns(updatedInfo, leftCol, midCol, rightCol, channelUrl) {
    ...
    // Fetch and add playlists box
    const playlists = await fetchPlaylistsData(channelUrl);
    if (playlists && playlists.length > 0) {
      const playlistsBox = createPlaylistsBox(playlists);
